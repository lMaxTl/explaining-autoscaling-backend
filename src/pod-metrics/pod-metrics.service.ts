...
} from 'src/schema/podInformation.schema';
...
export class PodMetricsService {
  ...
  constructor(
    @InjectModel(PodInformation.name)
    private podInformationModel: Model<PodInformationDocument>,
  ) {
  ...
  async savePodInformation() {
    ...
    for (const deployment of deployments) {
      const pods = await this.getPodsInDeployment(
  ...
  async getSavedPodInformation(
    ...
    const podInformation = await this.podInformationModel
      .find({
        podName: podName,
        namespace: namespace,
  ...
  async getPodsInDeployment(deploymentName: string, namespace: string) {
    const replicaSetName = await this.getReplicaSetOfDeployment(
      deploymentName,
      namespace,
    );
   ...
   * @returns
   */
  private async getPodsFromReplicaSet(
    namespace: string,
  ...
  async getReplicaSetOfDeployment(deploymentName: string, namespace: string) {
    ...
    const labelSelector = Object.keys(labels)
      ...
      .join(',');
    const replicaSetList = await k8sApi.listNamespacedReplicaSet(
      namespace,
      undefined,
      undefined,
      undefined,
