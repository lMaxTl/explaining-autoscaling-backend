...
export class PodMetricsController {
  ...
  async getPodContainerInformation(
    @Param('namespace') namespace: string,
    @Param('deploymentName') deployment: string,
    @Param('timestamp') timestamp: string,
  ) {
    const podList = await this.podMetricsService.getPodsInDeployment(
      deployment,
      namespace,
    );
    const podMetrics = [];
    for (const pod of podList) {
      const podInformation =
        await this.podMetricsService.getSavedPodInformation(
          pod.metadata.name,
          namespace,
          new Date(timestamp),
        );
