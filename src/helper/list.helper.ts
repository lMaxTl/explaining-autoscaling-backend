...
export function filterResult(filters: CrudFilters[], list: any) {
    ...
    filters.forEach(filter => {
        if (filter.operator == 'eq') {
            ...
            result = result.filter(item => filter.value.includes(item[filter.field]));
        }
            ...
            result = result.filter(item => item[filter.field].includes(filter.value));
        }
        else if (filter.operator == 'ncontainss') {
            result = result.filter(item => !item[filter.field].includes(filter.value));
        }
        else if (filter.operator == 'between') {
            result = result.filter(item => item[filter.field] >= filter.value[0] && item[filter.field] <= filter.value[1]);
        }
            ...
            result = result.filter(item => item[filter.field].toLowerCase().startsWith(filter.value.toLowerCase()));
        }
        else if (filter.operator == 'nstartswith') {
            result = result.filter(item => !item[filter.field].toLowerCase().startsWith(filter.value.toLowerCase()));
        }
        else if (filter.operator == 'startswiths') {
            result = result.filter(item => item[filter.field].startsWith(filter.value));
        }
        else if (filter.operator == 'nstartswiths') {
            result = result.filter(item => !item[filter.field].startsWith(filter.value));
        }
        else if (filter.operator == 'endswith') {
            result = result.filter(item => item[filter.field].toLowerCase().endsWith(filter.value.toLowerCase()));
        }
        else if (filter.operator == 'nendswith') {
            result = result.filter(item => !item[filter.field].toLowerCase().endsWith(filter.value.toLowerCase()));
        }
        else if (filter.operator == 'endswiths') {
            result = result.filter(item => item[filter.field].endsWith(filter.value));
        }
