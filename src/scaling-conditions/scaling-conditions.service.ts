...
} from 'src/schema/scalingConditions.schema';
...
export class ScalingConditionsService {
  ...
  constructor(
    @InjectModel(ScalingCondition.name)
    private conditionModel: Model<ScalingConditionDocument>,
  ) {
  ...
  async getScalingConditions(name: string, namespace: string): Promise<any> {
    return this.conditionModel
      .findOne({ deployment: name, namespace: namespace })
      .exec();
  }
  ...
  async retrieveScalingConditions() {
    const k8sApi = this.kubernetesConfig.makeApiClient(
      k8s.AutoscalingV2beta2Api,
    );
    const scalingConditions = await k8sApi
   ...
   */
  private async saveScalingConditions(
    scalingCondition: k8s.V2beta2HorizontalPodAutoscaler,
  ) {
    const scalingConditionDocument = new this.conditionModel();

    const scalingConditionStatusConditions = scalingCondition.status.conditions;
    for (const condition of scalingConditionStatusConditions) {
      const lastSavedScalingCondition = await this.conditionModel
        .findOne({ uid: scalingCondition.metadata.uid })
        .exec();

      if (lastSavedScalingCondition) {
        //find the condition in last saved condition with the same type
        const lastSavedCondition = lastSavedScalingCondition.conditions.find(
        ...
        if (lastSavedCondition) {
          ...
          if (
            new Date(lastSavedCondition.lastTransitionTime) <
