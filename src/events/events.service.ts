...
export class EventsService {
  ...
  async receiveNewAdaptionEvent(eventDto: EventDto) {
    ...
    if (eventDto.details == null) return;
    ...
    if (eventDto.details.reason === 'SuccessfulRescale') {
      ...
      if (event.scalingType == 'scaleOut') {
        ...
        event.metricValue = await this.getAllMetricValues(
          event.name,
          event.namespace,
        );
      }
      ...
      await this.isRelated(event).then(function (v) {
        isRelated = v;
      });
      if (isRelated) {
  ...
  async getMetricValue(
    ...
    const hpaConfigs =
      await this.hpaService.getHpaConfigurationByDeploymentName(
        name,
        namespace,
      );
    let metricValue = 0;
    for (const hpaConfig of hpaConfigs.currentMetrics) {
      if (hpaConfig.metricName === metricType) {
        metricValue = await this.prometheusService.queryPrometheus(
          hpaConfig.query,
        )[1];
        break;
      }
  ...
  async getAllMetricValues(name: string, namespace: string): Promise<number[]> {
    ...
    const metricValues = [];
    for (const hpaConfig of hpaConfigs.currentMetrics) {
      const metricValue = await this.prometheusService.queryPrometheus(
        hpaConfig.query,
      )[1];
   ...
   * @returns
   */
  async getLatestScaleOutEvent(
    name: string,
    namespace: string,
  ): Promise<Event> {
    return this.eventModel
      .findOne({ name: name, namespace: namespace, scalingType: 'scaleOut' })
      .sort({ createdAt: -1 })
      .exec();
  }
  ...
  extractScalingType(event: Event): string {
    const regexScaleOut = /(above)/g;
    const regexScaleIn = /(below)/g;
    let scalingType = '';
  ...
  extractReplicaSize(event: Event): number {
    const regexReplicaSize = /New size\: (\d+)/;
    let replicaSize = NaN;
    try {
  ...
  extractMetricType(event: Event): string {
    const regexMetricTypeScaleIn = /(reason: All metrics below target)/;
    const regexMetricType = /reason: (.*) metrics above target/;
    const regexExternalMetric = /external metric/;
    const regexMetricTypeExternalMetric = /(?<=tomato)(.*?)(?=tomato)/;
    let metricType = '';
    try {
      if (regexMetricTypeScaleIn.test(event.message)) {
        const metricType = 'scaleIn';
        return metricType;
  ...
  async isRelated(event: Event): Promise<boolean> {
    ...
    await this.getLatestEvent(event.name, event.namespace).then(function (v) {
      latestEvent = v;
    });
    if (latestEvent === null) {
      return false;
  ...
  isRelatedByTime(event: Event, latestEvent: Event): boolean {
    const relationTime = 300000;
    if (latestEvent === null) {
      return false;
    }
    const currentDate = new Date(event.createdAt);
    const latestDate = new Date(latestEvent.createdAt);
    const difference = Math.abs(currentDate.getTime() - latestDate.getTime());
  ...
  async isRelatedByDerivative(event: Event, latestEvent: Event) {
    ...
    const latestEventTime = new Date(latestEvent.createdAt).getTime();
    let isRelatedByDerivative: boolean;
  ...
  isRelatedByScalingType(event: Event, latestEvent: Event): boolean {
    if (
      event.scalingType === latestEvent.scalingType &&
      event.scalingType != 'TBD'
    ) {
